...
- Apple/macOS 本地 OCR、Gemini、OCR JSON 等只可以做提示；最後仍以 PDF 圖像為準。Tesseract/Tessdata 係硬性禁用：任何情況下都唔好安裝、調用、等待、讀取或採納相關輸出，因為佢喺呢類歷史中文直排材料上效果太差，會製造誤導。
...
6. 需要新 OCR/補漏時：
   ...
   - 其他 OCR 工具只當提示；Tesseract/Tessdata 例外，係硬性禁用，任何情況下都唔用。
7. 改文前先做機械掃描：
